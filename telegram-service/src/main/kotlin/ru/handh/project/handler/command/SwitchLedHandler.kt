...
class SwitchLedHandler(
    ...
    override fun isTriggered(update: Update) =
        isReply(update.message?.replyToMessage?.text, DeviceCommand.SWITCH_LED.replyMessage())

    override fun handle(update: Update, authChat: ChatEntity?) {
        commandHandlerHelper.handleDeviceCommand(update, DeviceCommand.SWITCH_LED)
    }
}
