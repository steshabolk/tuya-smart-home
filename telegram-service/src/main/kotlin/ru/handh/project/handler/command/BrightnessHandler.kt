...
class BrightnessHandler(
    ...
    override fun isTriggered(update: Update) =
        isReply(update.message?.replyToMessage?.text, DeviceCommand.BRIGHTNESS.replyMessage())

    override fun handle(update: Update, authChat: ChatEntity?) {
        commandHandlerHelper.handleDeviceCommand(update, DeviceCommand.BRIGHTNESS)
    }
}
