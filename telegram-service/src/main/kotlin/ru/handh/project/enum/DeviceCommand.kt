...
enum class DeviceCommand(
    ...
    COLOR(ActionType.COLOR) {
        ...
        override fun parseValue(value: String): ColorData.ColorHSV? {
            ...
            return ColorData.ColorHSV(hue = values[0], saturation = values[1], value = values[2])
        }
    },
    ...
    val inlineButton = inlineButton(this.actionType.description, this.actionType.command)
    ...
    companion object {
        ...
        fun from(s: String): DeviceCommand? = DeviceCommand.values().find { it.actionType.command == s }

        fun commandsKeyboard(rowSize: Int) =
            CommandCodeGen.values().mapNotNull {
                it.toCommandButton()
            }.getMultilevelKeyboard(rowSize)

        fun CommandCodeGen.toCommandButton() =
            convertMap.entries.associate { (k, v) -> v to k }[this]?.inlineButton
