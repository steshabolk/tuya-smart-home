...
class UserController(
    ...
    fun refresh(@RequestBody refreshTokenDto: RefreshTokenDto) =
        userClient.refresh(
            apiGatewayService.getUserId(),
            RefreshRequestGen(apiGatewayService.checkJwtIdsMatch(refreshTokenDto.refreshToken!!))
        )

    @PostMapping("/signout")
    fun signOut() =
        userClient.signOut(SignOutRequestGen(apiGatewayService.getJwtId()))

    @DeleteMapping("/account")
    fun deleteAccount(@RequestBody deleteAccountRequest: DeleteAccountRequestGen) =
        userClient.deleteAccount(apiGatewayService.getUserId(), deleteAccountRequest)

    @GetMapping("/telegram")
    fun getTelegramToken() =
        userClient.getTelegramToken(apiGatewayService.getUserId())
