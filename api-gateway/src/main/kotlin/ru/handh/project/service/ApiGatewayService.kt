...
class ApiGatewayService(
    ...
    final val pathsToSetUserIdParam =
    ...
    final
    val pathsToSetJwtIdParam = mapOf(
        "/api/refresh" to listOf(RequestMethod.POST),
        "/api/signout" to listOf(RequestMethod.POST),
    )

    final
    val pathsToCheckTokenExpiration =
        devicePath.toMap(allMethods) + homePath.toMap(allMethods) +
                mapOf(
                    "/api/signout" to listOf(RequestMethod.POST),
                    "/api/account" to listOf(RequestMethod.DELETE),
    ...
    final val pathsToCheckTokenHeader = pathsToSetUserIdParam + pathsToSetJwtIdParam
